...
const requestreject = async (req, res) => {
    try {
        ...
        const { id } = req.params;
       const data = await  contractor.findByIdAndUpdate(id,{approvalStatus: "Rejected"});
        res.status(200).json(data); 
    } catch (error) {
    ...
    const requestapprove = async (req, res) => {   try {
        console.log("requestapprove");
        const { id } = req.params;
       const data = await  contractor.findByIdAndUpdate(id,{approvalStatus: "Approved"});
        res.status(200).json(data); 
    } catch (error) {
...
module.exports = { requeststepone, requestreject, requestapprove, allcontractors, blockcontractor, unblockcontractor };